...
{
    const float walkingSpeed = 200; // Standard walking speed, in game units per second.
    ...
    bool standingOnIceTile, standingOnHotTile, standingOnSpeedTile, standingOnSlowTile; // Whether or not the character is standing on an ice tile or a hot tile.
    ...
    public override void Reset()
    {
        ...
        facingLeft = false;
        isGrounded = true;
        standingOnIceTile = standingOnHotTile = standingOnSpeedTile = standingOnSlowTile = false;
    ...
    public override void Update(GameTime gameTime)
    {
        ...
        if (IsAlive)
        {
            ...
            if (speedUpTimer > 0)
            {
                speed = 4 * walkingSpeed;
                speedUpTimer -= gameTime.ElapsedGameTime.TotalSeconds;
            }
            else if (slowDownTimer > 0)
            {
                speed = walkingSpeed;
                slowDownTimer -= gameTime.ElapsedGameTime.TotalSeconds;
            }
